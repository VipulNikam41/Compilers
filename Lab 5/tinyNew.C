...
public:
  ...
  void setregi(int reg, int i)
  {
  	registeri[reg]=i;
  }
  void setregr(int reg, float r)
  {
  	registerr[reg]=r;
  }
  int regi(int reg)
  {
  	return registeri[reg];
  }
  float regr(int reg)
  {
  	return registerr[reg];
  }
  ...
  int getFreeTime(int reg) 
  { 
  	return registerFreeTime[reg]; 
  }
...
namespace opcodes 
{
	enum optype 
	{
		var,str, label, move, addi,addr, subi,subr, muli,mulr, divi,divr, inci, deci, cmpi, push, pop, ret, link, unlnk, cmpr, jsr, jmp, jgt, jlt, jge, jle, jeq, jne, sys, endofprogram, emptyline, unknown 
	};
}
enum operandtype 
{
	id,stackref,reg,num,strval,empty,nonknown
};
enum syscalls 
{
	SCreadi,SCreadr,SCwritei,SCwriter,SCwrites,SChalt,SCunknown
};
...
public:
  StackElement(int ii, float ff)
  {
  	freeTime = 0; ivalue=ii; rvalue=ff; elementtype=dataitem;
  }
  StackElement(int fp)
  {
  	freeTime = 0; ivalue=fp; elementtype=fpitem;
  }
  StackElement(list<opcode>::iterator adr)
  {
  	freeTime = 0; address=adr; elementtype=adritem;
  }
  void setival(int i)
  {
  ...
  int ival()  
  {
  	passert(elementtype==dataitem,"illegal int stack reference"); 
    return ivalue;
  }
  float rval()
  {
  	passert(elementtype==dataitem,"illegal float stack reference"); 
    return rvalue;
  }
  int fval()
  {
  	passert(elementtype==fpitem,"illegal fp stack reference"); 
    return ivalue;
  }
  // vvvvvvvvvvvvvvvvvvvvvvvv Statistics vvvvvvvvvvvvvvvvv
  int getFreeTime()
...
public:
  Symbol(string s)
  {
  	freeTime=0; nam=s; ivalue=0;rvalue=0;sconst=false;
  }
